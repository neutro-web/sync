...
describe("Q2 · Model C: concurrent conflict detected, held, apply() synchronous", () => {
	...
	it("snapshot() shows last-confirmed value immediately after conflict detection", async () => {
		...
		const vA = clockA.mint();
		await engine.apply(
			makeStateBatch(scope, "u1", "confirmed-val", "id-A", vA),
		);

		const vB = clockB.mint();
		await engine.apply(
			makeStateBatch(scope, "u1", "concurrent-val", "id-B", vB),
		);
...
describe("Q5 · defer: conflict stays open, state unchanged, subsequent resolution lands", () => {
	...
	it("resolveConflict on a unit with no open conflict is a no-op and does not throw", () => {
		const engine = new Engine(new VectorClockStrategy("engine"));
		expect(() =>
			engine.resolveConflict(makeScope("no-conflict"), makeConflictUnit("u1"), {
				decision: "take-local",
			}),
		).not.toThrow();
	});
	...
	it("resolveConflict with decision 'merged' throws when strategy lacks mergeVersions", async () => {
		...
		const minimalClock: import("../../src/core/types.ts").ClockStrategy = {
			...
			compare(
				a: import("../../src/core/types.ts").Version,
				b: import("../../src/core/types.ts").Version,
			) {
		...
		expect(() =>
			engine.resolveConflict(scope, makeConflictUnit("u1"), {
				decision: "merged",
				// biome-ignore lint/suspicious/noExplicitAny: test-only cast to bypass type system for invalid decision
			} as any),
	...
	it("take-remote resolves an ephemeral conflict and does not advance the cursor", async () => {
		...
		await engine.apply({
			...
			changes: [
				{
					id: makeChangeId("id-A"),
					kind: "state",
					scope,
					unit: makeConflictUnit("u1"),
					value: "val-A",
					version: vA,
					lifetime: { class: "ephemeral" },
...
describe("Q3 · ResolverPump: resolver IS invoked on a concurrent conflict", () => {
	...
	it("async resolver rejection logs error and leaves conflict open", async () => {
		...
		expect(String(errors[0])).toContain(
			"[ResolverPump] async resolution failed",
		);
		const snap = await engine.snapshot(scope);
...
describe("Q4 · Convergence: deterministic resolver produces same result on both replicas", () => {
	...
	it("convergence holds across multiple units with independent concurrent conflicts", async () => {
		...
		await engineA.apply(
			makeStateBatch(scope, "u1", "u1-val-A", "id-u1-A", clockA.mint()),
		);
		await engineA.apply(
			makeStateBatch(scope, "u2", "u2-val-A", "id-u2-A", clockA.mint()),
		);
		await engineB.apply(
			makeStateBatch(scope, "u1", "u1-val-B", "id-u1-B", clockB.mint()),
		);
		await engineB.apply(
			makeStateBatch(scope, "u2", "u2-val-B", "id-u2-B", clockB.mint()),
		);
