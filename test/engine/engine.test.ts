...
describe("P2 · LWW contention: higher version wins everywhere", () => {
	it("two replicas write the same unit; after drain the higher version wins on both", async () => {
		...
		await engine0.apply(
			makeStateBatch(scope, "u1", "from-R0-low", "r0", v_low),
		);
		await engine1.apply(
			makeStateBatch(scope, "u1", "from-R1-high", "r1", v_high),
		);
	...
	it("convergence holds under repeated concurrent writes (3 rounds of contention)", async () => {
		...
		for (let round = 0; round < 3; round++) {
			...
			await engine0.apply(
				makeStateBatch(scope, "u1", `low-round-${round}`, `r0-${round}`, v_low),
			);
			await engine1.apply(
				makeStateBatch(
					scope,
...
describe("P4 · T3: ephemeral changes are off the durable path", () => {
	it("P4a: cursor does not advance on ephemeral changes", async () => {
		...
		await engine.apply(
			makeStateBatch(scope, "u1", "durable-val", "d1", clock.mint()),
		);
...
describe("P9 · 3-replica contention under partition", () => {
	it("all 3 replicas converge on globally-highest version after partition/reconnect", async () => {
		...
		await engines[2]!.apply(
			makeStateBatch(scope, "u1", "val-island", "r2-island", v_island),
		);
		// biome-ignore lint/style/noNonNullAssertion: i < n === engines.length
		await engines[0]!.apply(
			makeStateBatch(scope, "u1", "val-winner", "r0-winner", v_winner),
		);
...
describe("P10 · LWW cross-instance tiebreaking: _node breaks equal-_ts ties", () => {
	it("two instances minting the same _ts converge on higher-_node value", async () => {
		...
		await engine0.apply(
			makeStateBatch(scope, "u1", "val-node100", "tie-r0", v0),
		);
		await engine1.apply(
			makeStateBatch(scope, "u1", "val-node200", "tie-r1", v1),
		);
...
describe("P11 · Ephemeral preserves durable base in durableStateUnits", () => {
	it("ephemeral winning in snapshot() does not evict the durable entry from changes()", async () => {
		...
		// Durable write first.
		await engine.apply(
			makeStateBatch(scope, "u1", "durable-val", "d1", vDurable),
		);
		...
		await engine.apply(
			makeStateBatch(scope, "u1", "durable-val-2", "d2", vDurable2),
		);
