 ...
 */
import type { PersistenceStore, PersistenceRecord } from "../core/persistence.ts";
...
export class IndexedDBStore implements PersistenceStore {
  ...
  async appendChange(scopeKey: string, record: PersistenceRecord): Promise<void> {
    const db = await this._open();
    const tx = db.transaction("changes", "readwrite");
    await idbReq(tx.objectStore("changes").put({ scopeKey, seq: record.seq, change: record.change }));
  }

  async readChanges(scopeKey: string, since?: number): Promise<PersistenceRecord[]> {
    const db = await this._open();
    const tx = db.transaction("changes", "readonly");
    const lower: IDBValidKey = since !== undefined ? [scopeKey, since] : [scopeKey, 0];
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      req.onsuccess = () => {
        ...
        const row = cursor.value as { scopeKey: string; seq: number; change: Change };
  ...
  async clear(): Promise<void> {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      req.onblocked = () => { /* wait for onsuccess */ };
    });
  }
}
